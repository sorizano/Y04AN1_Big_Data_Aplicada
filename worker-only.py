"""
Breast Cancer (sintético) con Spark ML - Modo seguro para Windows
- Sesión Spark robusta (IPv4, tmp sin espacios, mismo Python del venv)
- Genera data sintética (pandas)
- pandas -> Spark DataFrame (schema explícito)
- Pipeline: Assembler -> Scaler -> LogisticRegression
- Cross-Validation (k=5), parallelism=1 (compatible con local[1])
- Métricas en test: AUC, Accuracy, Precision, Recall, F1 + matriz de confusión
- Exporta Excel con datos + predicciones + métricas en la misma carpeta
...
os.environ["SPARK_LOCAL_IP"] = "127.0.0.1"  # IPv4 estable

SPARK_TMP = r"C:\spark-tmp" if os.name == "nt" else "/tmp/spark-tmp"
Path(SPARK_TMP).mkdir(parents=True, exist_ok=True)

# =============================
# 1) Crear Sesión Spark (modo seguro)
# =============================
spark = (
    SparkSession.builder
    .appName("BreastCancerSyntheticCV-Safe")
    .master("local[1]")  # para diagnóstico; luego puedes pasar a local[*]
...
cv = CrossValidator(
    ...
    evaluator=evaluator,
    numFolds=5,
    parallelism=1,   # importante para local[1]; cuando pases a local[*], sube este valor
    seed=2025
)
...
# Extrae el LogisticRegressionModel entrenado
lr_model = None
for st in best_model.stages:
    if isinstance(st, LogisticRegressionModel):
        lr_model = st
        break
if lr_model is None and isinstance(best_model.stages[-1], LogisticRegressionModel):
    lr_model = best_model.stages[-1]
if lr_model is None:
    raise RuntimeError("No se encontró LogisticRegressionModel en el pipeline entrenado.")

print("CV completada. Mejor LR -> regParam:",
      lr_model.getRegParam(), "| elasticNetParam:", lr_model.getElasticNetParam())
